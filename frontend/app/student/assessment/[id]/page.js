...
export default function StudentAssessmentPage() {
  ...
  const fetchAssessment = async () => {
    try {
      const [assessmentRes, questionsRes] = await Promise.all([
        fetch(`https://anrf-backend.onrender.com/assessment/view/${params.id}`),
        fetch(`https://anrf-backend.onrender.com/assessment/questions/${params.id}`),
      ]);
  ...
  const submitAssessment = async () => {
    try {
      ...
      const response = await fetch("https://anrf-backend.onrender.com/submission/submit", {
        method: "POST",
