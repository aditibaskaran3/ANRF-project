...
export default function AdminDashboard() {
  ...
  const fetchFaculty = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/auth/faculty");
      if (res.ok) {
  ...
  const fetchAssessments = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/assessment/admin/all");
      if (res.ok) {
        const data = await res.json();
        setAssessments(data);
      }
  ...
  const fetchSubmissions = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/submission/all");
      if (res.ok) {
        const data = await res.json();
        setSubmissions(data);
      }
  ...
  const deleteFaculty = async (email) => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/auth/faculty/${encodeURIComponent(email)}`,
        { method: "DELETE" }
      );
      if (res.ok) {
